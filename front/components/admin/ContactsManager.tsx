  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this contact message?")) {
      try {
        ...
        // Update this URL to your deployed backend contacts endpoint (HTTPS)
        const response = await fetch(
          `https://wits26.science-conf.net/api/contacts/${id}`, // <-- CHANGE TO HTTPS
          {
            method: "DELETE",
            headers: {
    ...
    if (
      ...
      try {
        ...
        const deletePromises = filteredContacts.map(async (contact) => {
          // Update this URL to your deployed backend contacts endpoint (HTTPS)
          const response = await fetch(
            `https://wits26.science-conf.net/api/contacts/${contact.id}`, // <-- CHANGE TO HTTPS
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
