...
// Now that you are forcing HTTPS, this should strictly be HTTPS.
header('Access-Control-Allow-Origin: https://wits26.science-conf.net'); // <-- STRICTLY HTTPS
...
switch ($resource) {
    case '':
        sendJsonResponse(['message' => 'Welcome to the WITS2026 API! Available endpoints: /auth, /speakers, /committees, /contacts, /registrations, /upload_image, /upload_receipt']);
        break;
    ...
    case 'upload_receipt':
        if ($method === 'POST') {
            $uploadController = new UploadController();
            $uploadController->upload('receipt');
        } else {
