// fileName: api.ts
const API_BASE_URL = "http://localhost/witsReact/api";
const API_KEY =
...
export const speakersApi = {
  ...
  create: (data: {
    ...
    institution: string;
    bio: string;
    image?: string | null; // Confirmed: Allows string, null, or undefined
...
export async function uploadImage(file: File): Promise<{ image_path: string }> {
  ...
  const response = await fetch(`${API_BASE_URL}/upload_image`, {
    method: "POST",
    headers: {
      // IMPORTANT: DO NOT set Content-Type for FormData. The browser sets it automatically.
      "X-API-Key": API_KEY,
    },
    body: formData,
  });
  ...
  // Ensure the response from the server is parsed correctly and matches the expected type
  const result = await response.json();
